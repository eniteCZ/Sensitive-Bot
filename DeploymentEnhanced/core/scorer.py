...
#Addition
def INS_punisher(index,question,answer):
    text = answer
    verdict = 0
    modifier = score_settings['INS_modifier']
    INS = ["I'm not sure", "I am not sure", "I don't know", "not a fan"]
    for i in INS:
        if i in text:
            verdict = 1
    return (verdict * modifier)

def score_sentiment(index,question,answer):
    inp_sent = TextBlob(question).sentiment.polarity
    out_sent = TextBlob(answer).sentiment.polarity
    SD = math.sqrt((inp_sent - out_sent)**2)
    insult = False
    if SD <= 0.5:
        score = 2-SD * score_settings['sentiment_modifier']
    else:
        score = -(1+SD * score_settings['sentiment_modifier'])
    if inp_sent <= -0.4:
        if "sorry" in answer:
            score = score -2
    return score
